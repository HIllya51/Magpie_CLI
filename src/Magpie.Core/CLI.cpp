...
void loadeffects(ScalingOptions& options,const nlohmann::json& effects) {
    ...
    for (int i = 0; i < effects.size(); i++) {
        auto& ei = effects[i];
        EffectOption ef;
        ef.name = StrUtils::UTF8ToUTF16(ei["name"]);
        ...
        if (ei.find("parameters") != ei.end()) {
            ...
            for (auto& param : parameters.items())
                ef.parameters.insert(std::make_pair(StrUtils::UTF8ToUTF16(param.key()), param.value()));
        }
...
public:
    std::optional<T2>get(const T1& key) {
        ...
        if (_map.find(key) == _map.end())return {};
        return _map.at(key);
    }
...
public:
    ...
    LRESULT wndproc(UINT message, WPARAM wParam, LPARAM lParam) {
        ...
        if (!proc.has_value())return DefWindowProc(winId, message, wParam, lParam);
